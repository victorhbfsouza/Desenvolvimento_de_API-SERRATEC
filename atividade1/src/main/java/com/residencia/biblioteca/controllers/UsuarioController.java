...
public class UsuarioController {
	...
	public ResponseEntity<List<Usuario>> getAllUsuarios() {
		return new ResponseEntity<>(usuarioService.getAllUsuario(),
				HttpStatus.OK);
	}
	...
	public ResponseEntity<Boolean> deleteUsuario(@PathVariable Integer id) {
		Boolean usuarioDeletado =  usuarioService.deleteUsuario(id);
		
		if(usuarioDeletado) {
			return new ResponseEntity<>( usuarioDeletado, HttpStatus.OK);
		}
		else {
			return new ResponseEntity<>(usuarioDeletado, HttpStatus.NOT_MODIFIED);
 		}
